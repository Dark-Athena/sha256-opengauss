...
/*
    基于OpenGauss2.1.0的A兼容语法对Steve Jang开发的oracle pl/sql 包 sha256进行改写移植
    by DarkAthena 2022-04-26
    本仓库地址 https://github.com/Dark-Athena/sha256-opengauss
    原仓库地址 https://github.com/CruiserX/sha256_plsql
...
*/
    ...
    /* Initialize structure containing state of computation.
       (FIPS 180-2: 5.3.2)  */
    PROCEDURE sha256_init_ctx (ctx IN OUT  TR_CTX);
    ...
    PROCEDURE sha256_process_block (buffer IN number[],
                                    len IN NUMBER,
                                    ctx IN OUT  TR_CTX);
    ...
    function sha256_finish_ctx (ctx IN OUT  TR_CTX) return number[];
         ...
         select CASE WHEN 128 - left_over > t_len THEN t_len ELSE 128 - left_over END into add;
            ...
            FOR idx IN 1..add LOOP
                ...
                IF (left_over_mod=0) THEN
                    ctx.buffer32[left_over_blk] := BITAND(ctx.buffer32[left_over_blk],bits_00FFFFFF) + to_number(utl_raw.substr(t_buffer,idx,1),'xx')*16777216;
                ELSIF (left_over_mod=1) THEN
                    ctx.buffer32[left_over_blk] := BITAND(ctx.buffer32[left_over_blk],bits_FF00FFFF) + to_number(utl_raw.substr(t_buffer,idx,1),'xx')*65536;
                ELSIF (left_over_mod=2) THEN
                    ctx.buffer32[left_over_blk] := BITAND(ctx.buffer32[left_over_blk],bits_FFFF00FF) + to_number(utl_raw.substr(t_buffer,idx,1),'xx')*256;
                ELSE
                    ctx.buffer32[left_over_blk] := BITAND(ctx.buffer32[left_over_blk],bits_FFFFFF00) + to_number(utl_raw.substr(t_buffer,idx,1),'xx');
                END IF;
            END LOOP;
            ...
            BEGIN
                ...
                FOR idx IN 1..cnt LOOP
                    ...
                    IF (target_mod=0) THEN
                        x_buffer32[target_blk] := BITAND(x_buffer32[target_blk],bits_00FFFFFF) + to_number(utl_raw.substr(t_buffer,idx,1),'xx')*16777216;
                    ELSIF (target_mod=1) THEN
                        x_buffer32[target_blk] := BITAND(x_buffer32[target_blk],bits_FF00FFFF) + to_number(utl_raw.substr(t_buffer,idx,1),'xx')*65536;
                    ELSIF (target_mod=2) THEN
                        x_buffer32[target_blk] := BITAND(x_buffer32[target_blk],bits_FFFF00FF) + to_number(utl_raw.substr(t_buffer,idx,1),'xx')*256;
                    ELSE
                        x_buffer32[target_blk] := BITAND(x_buffer32[target_blk],bits_FFFFFF00) + to_number(utl_raw.substr(t_buffer,idx,1),'xx');
                    END IF;
                END LOOP;
